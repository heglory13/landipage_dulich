...
import { getServicePost, servicePosts } from "@/data/service-posts";
import { DatabaseArticle, getDatabaseArticle } from "@/components/database-article";

export const dynamic = "force-dynamic";

export function generateStaticParams() {
  return servicePosts.map((post) => ({ slug: post.slug }));
}
...
export default async function ServiceDetailPage({
  ...
  const stored = getDatabaseArticle("service", slug);
  if (stored?.status === "deleted" || stored?.status === "draft") notFound();
  if (stored) return <DatabaseArticle row={stored} listHref="/service" />;
          ...
          <aside className="space-y-5 self-start">
            <div className="bg-primary p-8 text-primary-foreground">
              ...
              <p className="mt-4 text-sm leading-7 text-primary-foreground/65">
                공항 VIP 패스트트랙과 여행 편의 서비스 안내를 모았습니다.
              </p>
            </div>
