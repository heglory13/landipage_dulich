...
const regions:Region[]=[
  ...
  {key:"vung-tau",label:"붕따우",prefixes:["/vung-tau/"],sections:[
    {key:"accommodation",label:"숙소 & 풀빌라",legacy:["vungtau"]},{key:"karaoke",label:"가라오케",legacy:["vungtaukaraoke"]},{key:"club",label:"클럽",legacy:["vungtauclub"]},{key:"bar",label:"바 & 주점",legacy:["vungtaubar"]},{key:"salon",label:"이발소 & 미용실",legacy:["vungtausalon"]},{key:"massage",label:"마사지",legacy:["vungtaumassage"]},{key:"golf",label:"골프",legacy:["vungtaugolf"]},{key:"travel",label:"여행지",legacy:["vungtautravel"]},{key:"restaurant",label:"맛집",legacy:["vungtaufood"]},
  ]},
  {key:"phu-quoc",label:"푸꾸옥",prefixes:["/phu-quoc/"],sections:[
    {key:"accommodation",label:"숙소 & 풀빌라",legacy:["phuquoc"]},{key:"karaoke",label:"가라오케",legacy:["phuquockaraoke"]},{key:"club",label:"클럽",legacy:["phuquocclub"]},{key:"bar",label:"바 & 주점",legacy:["phuquocbar"]},{key:"salon",label:"이발소 & 미용실",legacy:["phuquocsalon"]},{key:"massage",label:"마사지",legacy:["phuquocmassage"]},{key:"golf",label:"골프",legacy:["phuquocgolf"]},{key:"travel",label:"여행지",legacy:["phuquoctravel"]},{key:"restaurant",label:"맛집",legacy:["phuquocfood"]},
  ]},
...
function itemRegion(item:Item){
  ...
  const byHref=regions.find(region=>region.prefixes.some(prefix=>href.startsWith(prefix)));
  if(byHref)return byHref.key;
  const category=item.category??"";
  return regions.find(region=>region.sections.some(section=>section.legacy.includes(category)))?.key??"site";
}
...
export default async function ContentPage({searchParams}:{searchParams:Promise<Search>}){
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-7 md:px-7 lg:px-10 lg:py-10">
    ...
    {region?<nav key={region.key} data-region={region.key} className="mt-3 grid gap-2 rounded-md border border-[#c3c4c7] bg-white p-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5"><Link href={filterHref(query,{section:"all",page:"1"})} className={`rounded-md border px-4 py-3 text-sm font-semibold ${selectedSection==="all"?"border-[#1d2327] bg-[#1d2327] text-white":"border-[#dcdcde] bg-[#f6f7f7] hover:border-[#2271b1]"}`}>{region.label} 전체 ({regionCounts.get(region.key)??0})</Link>{region.sections.map(section=>{const count=allItems.filter(item=>itemRegion(item)===region.key&&itemSection(item,region)===section.key).length;return <Link key={`${region.key}-${section.key}`} href={filterHref(query,{section:section.key,page:"1"})} className={`rounded-md border px-4 py-3 text-sm font-semibold ${selectedSection===section.key?"border-[#2271b1] bg-[#2271b1] text-white":"border-[#dcdcde] bg-white hover:border-[#2271b1]"}`}>{fullSectionLabel(region,section)} ({count})</Link>})}</nav>:null}
