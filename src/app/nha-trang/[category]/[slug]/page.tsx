...
function fallbackArticleContent(post: ArticlePost) {
  ...
  const description = cleanText(post.summary).replace(title, "").replace(/\s*호치민 게임\s*/g, " ").trim();
  return `
    <h2>${title}</h2>
    ...
    <section className="w-full max-w-full overflow-hidden px-3 pb-14 pt-24 md:pt-28"><div className="mx-auto w-full max-w-[1280px]">
      ...
      <div className="grid min-w-0 max-w-full items-start gap-5 lg:grid-cols-[minmax(0,940px)_320px]">
        <article className="w-full min-w-0 max-w-full overflow-hidden rounded-[12px] bg-card px-5 py-6 shadow-[0_1px_2px_rgba(0,0,0,.04)]">
          <header className="mb-11 border-b border-border pb-6"><Link href={`/nha-trang/${rawCategory}`} className="text-xs text-muted-foreground">{category.label}</Link><h1 className="mt-3 font-sans text-[20px] font-bold leading-8">{post.title}</h1><div className="mt-4 text-xs text-muted-foreground">🎊 호치민 게임 · {postDate(post)}</div></header>
