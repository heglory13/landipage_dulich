...
const routeCategories = {
  accommodation: { source: null, label: "숙소&풀빌라", heading: "나트랑 숙소&풀빌라" },
  karaoke: { source: "nhatrangktv", label: "가라오케", heading: "나트랑 가라오케" },
  club: { source: "nhatrangclub", label: "클럽", heading: "나트랑 클럽" },
  bar: { source: null, label: "바&주점", heading: "나트랑 바&주점" },
  salon: { source: "nhatrangsalon", label: "이발소&미용실", heading: "나트랑 이발소&미용실" },
  massage: { source: "nhatrangmassage", label: "마사지", heading: "나트랑 마사지" },
  golf: { source: null, label: "골프", heading: "나트랑 골프" },
  travel: { source: null, label: "여행지", heading: "나트랑 여행지" },
  restaurant: { source: null, label: "맛집", heading: "나트랑 맛집" },
} as const;
...
const sidebarNavItems = [
  { key: "accommodation", label: "숙소&풀빌라", href: "/nha-trang/accommodation" },
  { key: "karaoke", label: "가라오케", href: "/nha-trang/karaoke" },
  { key: "club", label: "클럽", href: "/nha-trang/club" },
  { key: "bar", label: "바&주점", href: "/nha-trang/bar" },
  { key: "salon", label: "이발소&미용실", href: "/nha-trang/salon" },
  { key: "massage", label: "마사지", href: "/nha-trang/massage" },
  { key: "golf", label: "골프", href: "/nha-trang/golf" },
  { key: "travel", label: "여행지", href: "/nha-trang/travel" },
  { key: "restaurant", label: "맛집", href: "/nha-trang/restaurant" },
] as const;
...
const recentPosts = [
  ["Nha Trang Massage", "나트랑 나우스파 마사지 소개 및 예약방법", "최근", "/nha-trang/massage/나트랑-누루-불건마-나우스파-마사지-소개-및-4506"],
  ["Nha Trang Massage", "나트랑 클라우드 나인 마사지", "최근", "/nha-trang/massage/나트랑-불건마-마사지-클라우드-나인-massa-4357"],
  ["Nha Trang Karaoke", "나트랑 아노바 가라오케", "최근", "/nha-trang/karaoke/나트랑-가라오케-추천-아노바-가라오케-anno-4067"],
  ["Nha Trang Karaoke", "나트랑 두바이 가라오케", "최근", "/nha-trang/karaoke/나트랑-가라오케-추천-두바이-가라오케-kara-4034"],
  ["Nha Trang Salon", "나트랑 강남이발관", "최근", "/nha-trang/salon/나트랑-강남이발관-예약-여행-피로를-풀기-좋은-4174"],
  ["Nha Trang Club", "나트랑 돔 클럽", "최근", "/nha-trang/club/나트랑-클럽-추천-돔-클럽-dom-nha-tr-3857"],
] as const;

const recentComments = [
  ["Nha Trang Massage", "예약 문의드립니다", "3 phút trước", "/nha-trang/massage/나트랑-누루-불건마-나우스파-마사지-소개-및-4506"],
  ["Nha Trang Karaoke", "위치 확인 부탁드려요", "12 phút trước", "/nha-trang/karaoke/나트랑-가라오케-추천-아노바-가라오케-anno-4067"],
  ["Nha Trang Salon", "가격 정보 궁금합니다", "28 phút trước", "/nha-trang/salon/나트랑-강남이발관-예약-여행-피로를-풀기-좋은-4174"],
  ["Nha Trang Club", "주말 분위기 어떤가요?", "1 giờ trước", "/nha-trang/club/나트랑-클럽-추천-돔-클럽-dom-nha-tr-3857"],
] as const;
const eventItems = [
  ["2026년 5월 나트랑 호치민 게임 1:1 이벤트", "2026.04.29"],
  ["2025년 12월 나트랑 호치민 게임 정모 안내", "2025.11.20"],
  ["2025년 11월 나트랑 호치민 게임 정모 안내", "2025.10.23"],
  ["2025년 10월 나트랑 호치민 게임 이벤트", "2025.09.17"],
  ["2025년 9월 호치민 게임 정모 안내", "2025.08.22"],
  ["[호치민 게임 궁상] 나트랑 프라이빗 파티", "2025.04.16"],
] as const;
const serviceItems = [
...
const nhaTrangWeatherLocations = [
  { key: "seoul", name: "Seoul", latitude: 37.5665, longitude: 126.978 },
  { key: "nhaTrang", name: "Nha Trang", latitude: 12.2388, longitude: 109.1967 },
];
...
function matchesFilter(category: RouteCategory, post: ArchivedPost, filter: string) {
  ...
  if (category === "karaoke") return filter === "korean" ? /한인|한국|annova|아노바/i.test(text) : /로컬|dubai|두바이|win|윈|master|마스터|crown|크라운/i.test(text);
  if (category === "salon") {
...
export default async function NhaTrangCategoryPage({
  params,
  searchParams,
}: {
  ...
  const cmsPosts = getPublicCmsPosts([`nhatrang${rawCategory==="karaoke"?"ktv":rawCategory==="accommodation"?"room":rawCategory==="restaurant"?"food":rawCategory}`]);
    ...
    <section className="border-b border-border bg-secondary/50 px-4 pb-12 pt-28 sm:px-6 md:px-10 md:pb-16 md:pt-32">
      <div className="mx-auto max-w-[1740px]">
        <nav className="mb-8 flex items-center gap-2 text-xs tracking-[.12em] text-muted-foreground">
          <Link href="/">홈</Link><ChevronRight className="h-3.5 w-3.5" /><span>나트랑</span><ChevronRight className="h-3.5 w-3.5" /><span className="text-foreground">{category.label}</span>
        </nav>
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="min-w-0 space-y-5">
            ...
            <div className="rounded-2xl bg-card px-6 py-7 shadow-[0_10px_30px_rgba(30,26,20,.06)] md:px-10 md:py-8">
              <p className="text-xs tracking-[.28em] text-muted-foreground uppercase">Nha Trang · {category.label}</p>
              <h1 className="mt-3 font-serif text-3xl leading-tight md:text-5xl">Nha Trang <span className="cartoon-page-title italic text-accent">{category.label}</span></h1>
            </div>
          </div>
      ...
      <div className="mx-auto grid max-w-[1800px] gap-8 px-4 sm:px-6 md:px-10 lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="order-2 h-fit space-y-6 lg:order-1 lg:sticky lg:top-28">
          <div className="bg-card">
            <div className="border-b border-border px-6 py-6"><p className="text-xs tracking-[.25em] text-muted-foreground uppercase">Destination</p><h2 className="mt-2 font-serif text-2xl">Nha Trang</h2></div>
          ...
          <LiveTravelWidgets locations={nhaTrangWeatherLocations} />
        </aside>
        <div className="order-1 min-w-0 lg:order-2">
          <div className="mb-7 bg-card p-5 md:p-7">
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <nav className="flex flex-wrap gap-2 md:gap-3 xl:flex-nowrap" aria-label="나트랑 카테고리 필터">
